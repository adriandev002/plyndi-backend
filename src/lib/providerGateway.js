// ============================================================================
// Provider gateway — extracted unchanged from src/routes/generate.js so it can be called
// in-process by both POST /v1/generate (the thin HTTP wrapper it now is) and POST /v1/ai/run.
// A server calling itself over HTTP would double the latency, lose error detail, and break the
// moment the port or auth changes — so /v1/ai/run calls `generate()` below directly instead.
//
// BEHAVIOUR IS UNCHANGED FROM THE ORIGINAL generate.js: model chains, the circuit breaker and its
// cooldowns, error classification, the one-retry-per-model rule, the request deadline, and
// toOpenAISchema all moved across verbatim. See scripts/test-ai-run.js for the before/after proof
// on toOpenAISchema specifically.
// ============================================================================
...
// Thrown by generate() when the requested profile has no configured model chain.
class UnknownProfileError extends Error {
  constructor(profile) {
    super(`unknown_profile: ${profile}`);
    this.profile = profile;
  }
}

// Thrown by generate() when every model in the chain failed (or the deadline was hit before any
// succeeded). Carries the same fields the original route handler put straight into its 503 body.
class AllProvidersFailedError extends Error {
  constructor({ kind, latencyMs, attempts }) {
...
// ---------------------------------------------------------------------------
// generate() — the core loop, extracted verbatim from the old router.post('/') handler in
// generate.js, just turned into return/throw instead of res.json()/res.status().json().
//
// `providers` defaults to the real PROVIDERS map above but can be overridden by a caller (tests)
// with fakes, so scripts/test-ai-run.js and scripts/test-config.js-style scripts never need to
// reach the real network — this is the "injectable provider function" the Phase 1-A design calls
// for. Production callers (routes/generate.js, routes/aiRun.js) never pass this option.
// ---------------------------------------------------------------------------

async function generate({ profile = 'fast', system, user, jsonSchema, maxOutputTokens, temperature, providers = PROVIDERS }) {
  const chain = chainFor(profile);
  if (!chain?.length) throw new UnknownProfileError(profile);
  ...
  for (const { provider, model } of chain) {
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      ...
      try {
        ...
        return { text, provider, model, latencyMs: Date.now() - startedAt, attempts };
      } catch (error) {
        ...
        console.error(`[providerGateway] ${key} failed (${status}): ${error.message.slice(0, 200)}`);
  ...
  // Every model failed. The caller gets the CAUSE, not a generic message — the app needs
